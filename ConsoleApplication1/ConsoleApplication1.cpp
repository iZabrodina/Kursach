...
int main()
{
    ...
    double sumUnStock = 0, sumStock = 0, sumRatedTime = 0;         //критерии эффективности 
    ...
    myfile.close();
    cout << "\nВведите количество складируемых ресурсов:  ";
    int kolvoStock;
    cin >> kolvoStock;
    while (kolvoStock > kolvo_res)
    {
        cout << "\nКоличество складируемых ресурсов не может превышать общего количества ресурсов, повторите ввод:  ";
        cin >> kolvoStock;
    }
    int* jobStock = new int[kolvoStock];
    cout << "\nВведите  через пробел номера складируемых ресурсов, оставшиеся ресурсы пометятся как нескладируемые:  ";
    for (int i = 0; i < kolvoStock; i++)
    {
        cin >> jobStock[i];
        while (jobStock[i] > kolvo_res)
        {
            cout << "\nВведите корректный номер ресурса:  ";
            cin >> jobStock[i];
        }
    ...
    if (sost == true)
    {
        ...
        for (int i = 0; i < kolvo_res; i++)
        {
            ...
            for (int j = 0; j < kolvo_comb; j++)
            {
                ...
                if ((sostStock == true) && (resource[i][j] < ((y[j] - x[j] + 1) * z[i][j])))
                    sumStock += beta * ((((y[j] - x[j] + 1) * z[i][j]) - resource[i][j]) / resource[i][j]) * 100;
                if ((sostStock != true)) //&& (resource[i][j] < ((y[j] - x[j] + 1) * z[i][j])))
